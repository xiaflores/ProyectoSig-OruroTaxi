...
import com.example.proyectosig.models.ClientBooking;
...
public class RequestConductorActivity extends AppCompatActivity {
    ...
    private ClientBookingProvider mClientBookingPorvider;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mClientBookingPorvider=new ClientBookingProvider();
...
//cuando termina la busques de conductor de radio de 5 km
                        ...
                        public void onResponse(Call<FCMResponse> call, Response<FCMResponse> response) {
                            if(response.body()!=null) {
                                if(response.body().getSuccess()==1) {
                                    ...
                                    mClientBookingPorvider.create(clientBooking).addOnSuccessListener(new OnSuccessListener<Void>() {
                                        @Override
    ...
    private void checkStatusClientBooking(){
        mListener= mClientBookingPorvider.getStatus(mAuthProvider.getId()).addValueEventListener(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if(dataSnapshot.exists()){
                    String status=dataSnapshot.getValue().toString();
                    if(status.equals("accept")){
                        Intent intent=new Intent(RequestConductorActivity.this,MapClientBookingActivity.class);
    ...
    protected void onDestroy() {
        super.onDestroy();
        if(mListener!=null){
            mClientBookingPorvider.getStatus(mAuthProvider.getId()).removeEventListener(mListener);
        }
    }
}
